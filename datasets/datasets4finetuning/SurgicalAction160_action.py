...
class CLIPDataset_SurgicalAction160(Dataset):
    """Load video-level recognition dataset."""

    def __init__(
        self,
        anno_path="data/SurgicalAction160/train.txt",
        data_path="data/SurgicalAction160",
        mode="train",  # val/test
        clip_len=16,
        crop_size=224,
        short_side_size=256,
        ...
        with open(anno_path, "r") as f:
            infos = f.readlines()
        self.dataset_samples = [info.strip() for info in infos]
        self.label_array = [int(info.split('_')[0]) - 1 for info in self.dataset_samples]
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            buffer = self.load_video_data(os.path.join(self.data_path, "frames", info))
...
if __name__ == "__main__":
    ...
    dataset = CLIPDataset_SurgicalAction160(data_path="/project/smartlab2021/syangcw/SurgicalActions160", anno_path="/project/smartlab2021/syangcw/SurgicalActions160/train.txt")
