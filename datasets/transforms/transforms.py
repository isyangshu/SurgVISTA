...
class GroupMultiScaleTwoResizedCrop(object):
    def __init__(
        ...
        self.input_size_teacher = (
            input_size_teacher
            if not isinstance(input_size_teacher, int)
            else [input_size_teacher, input_size_teacher]
        )
        self.interpolation = Image.BILINEAR
    ...
    def __call__(self, img_tuple):
        ...
        ret_img_group_teacher = [
            img.resize((self.input_size_teacher[0], self.input_size_teacher[1]), self.interpolation)
            for img in crop_img_group
        ]
        return (ret_img_group, ret_img_group_teacher, label)
