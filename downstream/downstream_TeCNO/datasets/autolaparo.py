...
class AutoLaparoHelper(Dataset):
...
class AutoLaparo():
    def __init__(self, hparams):
        self.name = "AutoLaparoPickle"
        self.hparams = hparams
        self.out_features = self.hparams.out_features

        self.data_p = {}
        self.data_p["train"] = [(f"video_{i:02d}.pkl") for i in range(1, 11)]
        self.data_p["val"] = [(f"video_{i:02d}.pkl") for i in range(11, 15)]
        self.data_p["test"] = [(f"video_{i:02d}.pkl") for i in range(15, 22)]

        # Datasplit is equal to Endonet and Multi-Task Recurrent ConvNet with correlation loss for surgical vid analysis
        self.weights = {}
        self.weights["train"] = [1.0] * 7
        self.weights["train_log"] = [1.0] * 7

        self.data = {}
        for split in ["train", "val", "test"]:
            self.data[split] = AutoLaparoHelper(hparams,
                                              self.data_p[split],
                                              dataset_split=split)
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        autolaparo_specific_args = parser.add_argument_group(
            title='AutoLaparo dataset specific args options')
