...
class AutoLaparoFeatureExtract:
    def __init__(self, hparams):
        ...
        self.cholec_root_dir = Path(self.hparams.data_root +"/AutoLaparo")  # videos splitted in images
        ...
        self.vids_for_training = [i for i in range(1, 11)]
        self.vids_for_val = [i for i in range(11, 15)]
        self.vids_for_test = [i for i in range(15, 22)]
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        autolaparo_specific_args = parser.add_argument_group(
            title='AutoLaparo specific args options')
        autolaparo_specific_args.add_argument(
            "--dataset_mode",
            default='img',
            choices=[
                'vid', 'img'
            ])
        autolaparo_specific_args.add_argument("--test_extract", action="store_true")
        return parser
...
class Dataset_from_Dataframe_video_based(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"], self.samples[index]["frame_id"], self.samples[index]["frame_id"], img_list]
        ...
        for video_id in infos.keys():
            ...
            for index, line_info in enumerate(data):
                ...
                for i in frame_index_list:
                    img_path = os.path.join(self.img_root, line_info["video_id"], str(data_[i]["frame_id"]).zfill(5) + ".png")
...
class Dataset_from_Dataframe(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"], self.samples[index]["frame_id"], self.samples[index]["frame_id"]]
    ...
    def _make_dataset(self, infos):
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                img_path = os.path.join(self.img_root, line_info["video_id"], str(line_info["frame_id"]).zfill(5) + ".png")
