...
class Cataract101():
    def __init__(self, hparams):
        self.name = "Cataract101Pickle"
        self.hparams = hparams
        self.out_features = self.hparams.out_features

        self.data_p = {}
        self.data_p["train"] = [(f"video_{i:02d}.pkl") for i in range(1, 51)]
        self.data_p["val"] = [(f"video_{i:02d}.pkl") for i in range(51, 61)]
        self.data_p["test"] = [(f"video_{i:02d}.pkl") for i in range(61, 101)]

        # Datasplit is equal to Endonet and Multi-Task Recurrent ConvNet with correlation loss for surgical vid analysis
        self.weights = {}
        self.weights["train"] = [1.0] * 10
        self.weights["train_log"] = [1.0] * 10
