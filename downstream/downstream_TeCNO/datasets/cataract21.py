...
class Cataract21Helper(Dataset):
...
class Cataract21():
    def __init__(self, hparams):
        self.name = "Cataract21Pickle"
        self.hparams = hparams
        self.out_features = self.hparams.out_features

        self.data_p = {}
        self.data_p["train"] = [(f"video_{i:02d}.pkl") for i in range(1, 19)]
        self.data_p["val"] = [(f"video_{i:02d}.pkl") for i in range(19, 22)]
        self.data_p["test"] = [(f"video_{i:02d}.pkl") for i in range(19, 22)]

        # Datasplit is equal to Endonet and Multi-Task Recurrent ConvNet with correlation loss for surgical vid analysis
        self.weights = {}
        self.weights["train"] = [1.0] * 11
        self.weights["train_log"] = [1.0] * 11

        self.data = {}
        for split in ["train", "val", "test"]:
            self.data[split] = Cataract21Helper(hparams,
                                              self.data_p[split],
                                              dataset_split=split)
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        cataract21_specific_args = parser.add_argument_group(
            title='Cataract21 dataset specific args options')
