...
class CATARACT21FeatureExtract:
    def __init__(self, hparams):
        ...
        self.cholec_root_dir = Path(self.hparams.data_root + "/Cataract21")  # videos splitted in images
        self.transformations = self.__get_transformations()
        weights = [1.0] * 11
        self.class_weights = np.asarray(weights)
        self.label_col = "class"
        self.df = {}
        self.df["train"] = pd.read_pickle(self.cholec_root_dir / "labels_pkl/train/1fpstrain.pickle")
        self.df["val"] = pd.read_pickle(self.cholec_root_dir / "labels_pkl/val/1fpsval.pickle")
        self.df["test"] = pd.read_pickle(self.cholec_root_dir / "labels_pkl/test/1fpstest.pickle")
        self.df["all"] = {**self.df["train"], **self.df["test"]}
        self.vids_for_training = [i for i in range(1, 19)]
        self.vids_for_val = [i for i in range(19, 22)]
        self.vids_for_test = [i for i in range(19, 22)]
        ...
        if hparams.test_extract:
            ...
            self.vids_for_test = [i for i in range(1, 22)]
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        cataract21_specific_args = parser.add_argument_group(
            title='Cataract21 specific args options')
        cataract21_specific_args.add_argument(
            "--dataset_mode",
            default='img',
            choices=[
                'vid', 'img'
            ])
        cataract21_specific_args.add_argument("--test_extract", action="store_true")
        return parser
...
class Dataset_from_Dataframe_video_based(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"], self.samples[index]["frame_id"], self.samples[index]["video_name"], img_list]
        ...
        for video_id in infos.keys():
            ...
            for index, line_info in enumerate(data):
                ...
                for i in frame_index_list:
                    img_path = os.path.join(self.img_root, line_info["video_name"], str(data_[i]["frame_id"]).zfill(5) + ".png")
...
class Dataset_from_Dataframe(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"], self.samples[index]["frame_id"], self.samples[index]["video_name"]]
    ...
    def _make_dataset(self, infos):
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                img_path = os.path.join(self.img_root, line_info["video_name"], str(line_info["frame_id"]).zfill(5) + ".png")
