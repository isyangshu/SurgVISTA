...
class CATARACTFeatureExtract:
    def __init__(self, hparams):
        ...
        self.cholec_root_dir = Path(self.hparams.data_root +"/CATARACT")  # videos splitted in images
        self.transformations = self.__get_transformations()
        weights = [1.0] * 19
        self.class_weights = np.asarray(weights)
        self.label_col = "class"
        self.df = {}
        train_pkl = pd.read_pickle(self.cholec_root_dir / "labels/train/1fpstrain.pickle")
        val_pkl = pd.read_pickle(self.cholec_root_dir / "labels/val/1fpsval.pickle")
        test_pkl = pd.read_pickle(self.cholec_root_dir / "labels/test/1fpstest.pickle")
        ...
        self.vids_for_training = [i for i in range(1, 26)]
        self.vids_for_val = [i for i in range(26, 31)]
        self.vids_for_test = [i for i in range(31, 51)]
        ...
        if hparams.test_extract:
            ...
            self.vids_for_test = [i for i in range(1, 51)]
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        cataract_specific_args = parser.add_argument_group(
            title='CATARACT specific args options')
        cataract_specific_args.add_argument(
            "--dataset_mode",
            default='img',
            choices=[
                'vid', 'img'
            ])
        cataract_specific_args.add_argument("--test_extract", action="store_true")
        return parser
