...
class Cholec80FeatureExtract:
    def __init__(self, hparams):
        ...
        self.cholec_root_dir = Path(self.hparams.data_root +"/cholec80")  # videos splitted in images
        ...
        self.df["val"] = pd.read_pickle(self.cholec_root_dir / "labels/test/1fpsval_test.pickle")
        self.df["test"] = pd.read_pickle(self.cholec_root_dir / "labels/test/1fpsval_test.pickle")
        ...
        if self.dataset_mode == "img":
            for split in ["train", "val", "test"]:
                self.data[split] = Dataset_from_Dataframe(
                    ...
                    img_root=self.cholec_root_dir / "frames_cutmargin",
                    split=split)
...
class Dataset_from_Dataframe_video_based(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"][5:], self.samples[index]["original_frame_id"], self.samples[index]["frame_id"], img_list]
        ...
        for video_id in infos.keys():
            ...
            for index, line_info in enumerate(data):
                ...
                for i in frame_index_list:
                    img_path = os.path.join(self.img_root, line_info["video_id"], str(data_[i]["original_frame_id"]) + ".jpg")
...
class Dataset_from_Dataframe(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"][5:], self.samples[index]["original_frame_id"], self.samples[index]["frame_id"]]
    ...
    def _make_dataset(self, infos):
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                img_path = os.path.join(self.img_root, line_info["video_id"], str(line_info["original_frame_id"]) + ".jpg")
