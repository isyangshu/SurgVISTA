...
class M2CAI16FeatureExtract:
    def __init__(self, hparams):
        ...
        self.cholec_root_dir = Path(self.hparams.data_root +"/M2CAI16-workflow")  # videos splitted in images
        ...
        self.df = {}
        self.df["train"] = pd.read_pickle(self.cholec_root_dir / "labels/train/1fpstrain.pickle")
        self.df["val"] = pd.read_pickle(self.cholec_root_dir / "labels/test/1fpstest.pickle")
        self.df["test"] = pd.read_pickle(self.cholec_root_dir / "labels/test/1fpstest.pickle")
        self.df["all"] = {**self.df["train"], **self.df["test"]}
        self.vids_for_training = [i for i in range(1, 28)]
        self.vids_for_val = [i for i in range(28, 42)]
        self.vids_for_test = [i for i in range(28, 42)]
        if hparams.test_extract:
            ...
            self.vids_for_test = [i for i in range(1, 42)]
    ...
    def add_dataset_specific_args(parser):  # pragma: no cover
        cholec80_specific_args = parser.add_argument_group(
            title='M2CAI16 specific args options')
...
class Dataset_from_Dataframe_video_based(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"][6:], self.samples[index]["frame_id"], self.samples[index]["frame_id"], img_list]
...
class Dataset_from_Dataframe(Dataset):
    ...
    def __getitem__(self, index):
        ...
        add_label = [self.samples[index]["video_id"][6:], self.samples[index]["frame_id"], self.samples[index]["frame_id"]]
