...
class EndoViT(nn.Module):
    def __init__(self, hparams):
        super(EndoViT, self).__init__()
    ...
    def add_model_specific_args(parser):  # pragma: no cover
        EndoViT_specific_args = parser.add_argument_group(
            title='EndoViT specific args options')

        # Model parameters
        EndoViT_specific_args.add_argument("--mae_model",
                                            default="vit_base_patch16",
                                            choices=["vit_base_patch16", "vit_large_patch16", "vit_huge_patch16"],
                                            help="MAE architecture to use")

        EndoViT_specific_args.add_argument('--drop_path', type=float, default=0.1, metavar='PCT',
                            help='Drop path rate (default: 0.1)')

        # Finetuning params
        EndoViT_specific_args.add_argument('--mae_ckpt', default='surgical_pretrain_params/endovit_SPR.pth',
                            help='Start training from an MAE checkpoint')

        EndoViT_specific_args.add_argument('--freeze_weights', type=int, default=-1, choices=[-1, 0, 1, 2, 3, 4, 5], 
            help=
            """
        ...
        EndoViT_specific_args.add_argument(
            "--model_specific_batch_size_max", type=int, default=128)

        # Last layer parameters
        EndoViT_specific_args.add_argument('--nb_classes', default=2048, type=int,
                            help='number of the features in the output of the last layer (head)') # set it to 0 to remove the last layer
        
        # LLRD
        EndoViT_specific_args.add_argument('--mae_layer_decay', type=float, default=0.65, # setting this to 1 disables it
                            help='layer-wise lr decay from ELECTRA/BEiT')

        # MAE weight_decay
        EndoViT_specific_args.add_argument('--mae_weight_decay', type=float, default=0.,
                            help="MAE model's weight decay (default: 0.)")

        # re-init MAE weights
        EndoViT_specific_args.add_argument('--mae_reinit_n_layers', type=int, default=-1,
                            help='Re-initialize last n layers of the encoder.')

        # return MAE optimizer parameter groups
        EndoViT_specific_args.add_argument('--return_mae_optimizer_groups', action='store_true',
                            help="""
                            ...
                            """)
        EndoViT_specific_args.set_defaults(return_mae_optimizer_groups=True)
