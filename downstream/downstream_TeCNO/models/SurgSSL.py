...
import models.SurgSSL_utils.models_vit as models_vit
from models.SurgSSL_utils.prepare_mae_model import prepare_mae_model
from timm.models.layers import trunc_normal_

class SurgSSL(nn.Module):
    def __init__(self, hparams):
        super(SurgSSL, self).__init__()
    ...
    def add_model_specific_args(parser):  # pragma: no cover
        SurgSSL_specific_args = parser.add_argument_group(
            title='EndoSSL specific args options')

        # Model parameters
        SurgSSL_specific_args.add_argument("--vit_model",
                                            default="SurgSSL",
                                            help="ViT architecture to use")

        SurgSSL_specific_args.add_argument('--drop_path', type=float, default=0.1, metavar='PCT',
                            help='Drop path rate (default: 0.1)')

        # Finetuning params
        SurgSSL_specific_args.add_argument('--mae_ckpt', default='surgical_pretrain_params/SurgSSL_E.pth',
                            help='Start training from an MAE checkpoint')


        SurgSSL_specific_args.add_argument(
            "--model_specific_batch_size_max", type=int, default=16)

        # Last layer parameters
        SurgSSL_specific_args.add_argument('--nb_classes', default=2048, type=int,
                            help='number of the features in the output of the last layer (head)') # set it to 0 to remove the last layer
        
        # LLRD
        SurgSSL_specific_args.add_argument('--mae_layer_decay', type=float, default=0.65, # setting this to 1 disables it
                            help='layer-wise lr decay from ELECTRA/BEiT')

        # MAE weight_decay
        SurgSSL_specific_args.add_argument('--mae_weight_decay', type=float, default=0.,
                            help="MAE model's weight decay (default: 0.)")

        # return MAE optimizer parameter groups
        SurgSSL_specific_args.add_argument('--return_mae_optimizer_groups', action='store_true',
                            help="""
                            ...
                            """)
        SurgSSL_specific_args.set_defaults(return_mae_optimizer_groups=True)
