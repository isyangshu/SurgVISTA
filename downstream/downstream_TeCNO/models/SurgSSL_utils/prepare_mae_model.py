...
import torch
import models.SurgSSL_utils.lr_decay as lrd
from timm.models import create_model
import models.SurgSSL_utils.models_vit as models_vit
from models.SurgSSL_utils.pos_embed import interpolate_pos_embed
"""
...
def prepare_mae_model(model_type, nb_classes, drop_path, ckpt, return_optimizer_groups=False, weight_decay=0., layer_decay=1.0, verbose=True, debug=False):
    # build model
    print("Configuring SurgSSL model:")
    ...
    if ckpt:
        ...
        print("Loaded pre-trained SurgSSL checkpoint from: %s" % ckpt)
    ...
    if (verbose):
        ...
        print("*" * 200)
        print("")
        print("SurgSSL Model = %s" % str(model))
        print("")
