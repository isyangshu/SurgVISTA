...
def write_pkl(root_dir):
    ...
    img_base_path = root_dir / "frames_cutmargin"
    ...
    for i in tqdm(range(1, 81)):
        vid_df = pd.DataFrame()
        if i in [j for j in range(1, 41)]:
            img_path_for_vid = img_base_path / "train" / f"video{i:02d}"
            img_list = sorted(img_path_for_vid.glob('*.jpg'))
        img_list = [str(i.relative_to(img_base_path / "train")) for i in img_list]
        else:
            img_path_for_vid = img_base_path / "test" / f"video{i:02d}"
            img_list = sorted(img_path_for_vid.glob('*.jpg'))
            img_list = [str(i.relative_to(img_base_path / "test")) for i in img_list]
    ...
    cholec_df.to_pickle(out_path / "cholec_split.pkl")


if __name__ == "__main__":
    pd.set_option('display.max_columns', None)
    root_dir = "/home/syangcw/cholec80/"
    write_pkl(root_dir)
