...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # 添加Cls token
        x = rearrange(x, "(b k) t c -> b (t k) c", b=B)  # Spatial-Temporal tokens
...
@register_model
def unified_base_image(pretrained=False, pretrain_path=None, **kwargs):
    if "wit400m" in pretrain_path:
        pre_norm = True
        patchembed_bias = False
    else:
        pre_norm = False
        patchembed_bias = True
    ...
    model = VisionTransformer(
        ...
        pre_norm=pre_norm,
        patchembed_bias=patchembed_bias,
        **kwargs,
    )
...
if __name__ == "__main__":
    ...
    model = unified_base_image(
        pretrained=True,
        pretrain_path="/home/syangcw/SurgSSL/pretrain_params/dino_base_imagenet1k.bin",
        qkv_bias = True,
        qkv_divided_bias = False,
        qkv_divided = False,
    )
