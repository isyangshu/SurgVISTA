...
class PretrainMaskedVideoStudent(nn.Module):
    def __init__(
        ...
        trunc_normal_(self.mask_token_img, std=0.02)

        # Decoder for video
        self.mask_token_vid = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
        self.down_vid = nn.Linear(encoder_embed_dim, decoder_embed_dim)
        self.decoder_vid = PretrainVisionTransformerDecoder(
            patch_size=patch_size,
            num_classes=decoder_num_classes,
            embed_dim=decoder_embed_dim,
            depth=decoder_depth_video,
        ...
        trunc_normal_(self.mask_token_vid, std=0.02)
    ...
    def forward(self, x, mask):
        ...
        expand_pos_embed_img = (
            self.pos_embed_img.type_as(x_vis_img).detach().expand(B, -1, -1)
        )
        pos_emd_vis_img = expand_pos_embed_img[~mask].reshape(B, -1, C)
        pos_emd_mask_img = expand_pos_embed_img[mask].reshape(B, -1, C)
        x_img = torch.cat(
            [
                x_vis_img[:, s:, :] + pos_emd_vis_img,
                self.mask_token_img + pos_emd_mask_img,
            ],
            dim=1,
        )  # [B, N, C_d]
        ...
        expand_pos_embed_vid = (
            self.pos_embed_vid.type_as(x_vis_vid).detach().expand(B, -1, -1)
        )
        pos_emd_vis_vid = expand_pos_embed_vid[~mask].reshape(B, -1, C)
        pos_emd_mask_vid = expand_pos_embed_vid[mask].reshape(B, -1, C)
        x_vid = torch.cat(
            [
                x_vis_vid[:, s:, :] + pos_emd_vis_vid,
                self.mask_token_vid + pos_emd_mask_vid,
            ],
            dim=1,
        )  # [B, N, C_d]
