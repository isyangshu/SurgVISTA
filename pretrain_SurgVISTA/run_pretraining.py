...
def get_args():
    ...
    parser.add_argument("--decoder_depth_video", default=8, type=int, help="depth of decoder")
    parser.add_argument("--decoder_depth_image", default=4, type=int, help="depth of decoder")
    ...
    parser.add_argument("--video_reconstruction_loss_weight", default=1.0, type=float)
    ...
    parser.add_argument(
        "--output_dir", default="/project/mmendoscope/SurgSSL_output", help="path where to save, empty for no saving"
    )
    parser.add_argument("--log_dir", default="/project/mmendoscope/SurgSSL_output", help="path where to tensorboard log")
    parser.add_argument(
...
def get_model(args):
    ...
    model = create_model(
        ...
        decoder_depth_video=args.decoder_depth_video,
        decoder_depth_image=args.decoder_depth_image,
...
def main(args):
    ...
    #     args.output_dir,
    #     "[{model}]-[Image-{Image_teacher}]-[{time}]-[{lr}]".format(
    #             model=args.model,
    #             Image_teacher=args.image_teacher_model,
    #             dataset="_".join(args.pretrained_datasets),
    ...
    args.output_dir = os.path.join(
        args.output_dir,
        "[{epochs}]-[{maskratio}]-[SurgeryMAEKD]-[{model}]-[{Image_teacher}]-[{dataset}]".format(
                model=args.model,
                Image_teacher=args.image_teacher_model,
                dataset="_".join(args.pretrained_datasets),
                maskratio=str(args.mask_ratio),
